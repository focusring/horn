...
| `docs/public/wasm/` | Not committed: `docs/scripts/sync-wasm.mjs` downloads the released `@focusring/horn-wasm` matching `docs/package.json` before every docs build. The live demo itself resolves the newest `@focusring/horn-wasm` from the npm registry at page load (files served by jsDelivr) and only uses this bundled copy as a fallback, so a new release reaches the demo without a docs rebuild |
