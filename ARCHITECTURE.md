...
| `docs/public/wasm/` | Not committed: `docs/scripts/sync-wasm.mjs` downloads the released `@focusring/horn-wasm` matching `docs/package.json` before every docs build. The live demo itself resolves the newest `@focusring/horn-wasm` from the npm registry at page load (files served by jsDelivr) and only uses this bundled copy as a fallback, so a new release reaches the demo without a docs rebuild. The version label always names the engine actually loaded: the npm version on the primary path, or `public/wasm/VERSION` (written by the sync script) on the fallback |
